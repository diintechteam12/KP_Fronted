      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <div>
              <p className="text-gray-500 text-sm mb-4">Follow on Social Media</p>
              <div className="flex gap-3">
                {socials.map(({ Icon, color, href }, i) => (
                  <motion.a key={i} href={href} target="_blank" rel="noopener noreferrer"
                    className="w-11 h-11 rounded-xl flex items-center justify-center text-white cursor-pointer"
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                {[
                  ...
                  <input key={f.name} name={f.name} type={f.type} placeholder={f.placeholder}
                    ...
                    className="w-full px-5 py-4 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-500 text-sm outline-none focus:border-yellow-400/50 focus:bg-white/8 transition-all duration-300" />
                ))}
                <textarea name="message" placeholder="Your Message" rows={4} value={form.message} onChange={onChange} required
                  className="w-full px-5 py-4 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-500 text-sm outline-none focus:border-yellow-400/50 focus:bg-white/8 transition-all duration-300 resize-none" />
