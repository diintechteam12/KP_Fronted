  ...
  return (
    <section id="contact" className="py-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <SectionTitle 
          subtitle={lang === 'hi' ? 'संपर्क करें' : 'Get in Touch'} 
          title={lang === 'hi' ? 'दरवाज़े हमेशा' : 'The Door Is'} 
          highlight={lang === 'hi' ? 'खुले हैं' : 'Always Open'}
          desc={lang === 'hi' ? 'कोई सवाल, कोई चिंता, या बस हमारे काम का हिस्सा बनना चाहते हैं — सीधे संपर्क करें।' : 'Any question, any concern, or just want to be part of what we are doing — reach out directly.'} 
        />
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 mb-8 bg-white/50 backdrop-blur-md border border-gray-100 p-6 md:p-8 rounded-3xl shadow-xl max-w-5xl mx-auto">
          ...
          <motion.div className="flex-1 w-full" initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <h3 className="text-gray-900 text-2xl font-bold mb-4" style={{ fontFamily: 'Playfair Display,serif' }}>
              {lang === 'hi' ? 'आपका यहाँ स्वागत है' : 'You Are Welcome Here'}
            </h3>
            <p className="text-gray-600 leading-relaxed mb-6">
              {lang === 'hi' ? 'बड़ा मुद्दा हो या छोटा — हम सुनते हैं। क्योंकि सच्ची सेवा ऐसी ही होती है।' : 'Big issue or small — we listen. Because that is what real service looks like.'}
            </p>
          ...
          <motion.div className="flex-1 w-full flex justify-center lg:justify-end" initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
             <motion.button 
               ...
               whileTap={{ scale: 0.95 }}>
               <FaPaperPlane />
               {lang === 'hi' ? 'अभी संपर्क करें' : 'Contact Now'}
             </motion.button>
          </motion.div>
        </div>
            ...
            >
              ...
              <div className="p-8">
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                  {[
                    { name: 'name', placeholder: lang === 'hi' ? 'आपका पूरा नाम' : 'Your Full Name', type: 'text' },
                    { name: 'phone', placeholder: lang === 'hi' ? 'फोन नंबर' : 'Phone Number', type: 'tel' },
                    { name: 'email', placeholder: lang === 'hi' ? 'ईमेल पता' : 'Email Address', type: 'email' },
                  ].map(f => (
                  ...
                  <textarea name="message" placeholder={lang === 'hi' ? 'आपका संदेश' : 'Your Message'} rows={4} value={form.message} onChange={onChange} required
                  ...
                  <motion.button type="submit" disabled={status.loading}
                    ...
                    whileTap={{ scale: 0.98 }}>
                    <FaPaperPlane />
                    {status.loading ? (lang === 'hi' ? 'भेजा जा रहा है...' : 'Sending...') : status.sent ? (lang === 'hi' ? 'संदेश भेजा गया ✓' : 'Message Sent ✓') : (lang === 'hi' ? 'संदेश भेजें' : 'Send Message')}
                  </motion.button>
                </form>
              </div>
            </motion.div>
