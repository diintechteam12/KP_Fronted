...
export default function About() {
  ...
  const localizedAbout = useLocalized(aboutData);
  ...
  return (
    <section id="about" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <SectionTitle subtitle={localizedAbout.sectionSubtitle} title={localizedAbout.sectionTitle} highlight={localizedAbout.sectionHighlight}
          desc={localizedAbout.sectionDesc} />
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div className="relative"
            ...
            <div className="relative rounded-3xl overflow-hidden" style={{ boxShadow: '0 30px 80px rgba(15,81,50,0.15)' }}>
              <img src={localizedAbout.imageUrl || '/Kp image.png'} alt={localizedAbout.sectionTitle} className="w-full object-cover" style={{ height: 520 }} />
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <div className="rounded-2xl px-5 py-3 backdrop-blur-xl bg-white/10 border border-white/10">
                  <p className="text-white font-semibold text-lg" style={{ fontFamily: 'Playfair Display,serif' }}>{localizedAbout.sectionTitle} {localizedAbout.sectionHighlight}</p>
                  <p className="text-sm text-yellow-400">{localizedAbout.sectionSubtitle}</p>
                </div>
              </div>
            </div>
            <motion.div className="absolute -top-6 -right-6 w-28 h-28 rounded-full flex flex-col items-center justify-center text-white font-bold"
              ...
              <span className="text-3xl leading-none">{localizedAbout.yearsOfService}</span>
          ...
          <motion.div initial={{ opacity: 0, x: 60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.9 }}>
            <p className="text-gray-600 text-lg leading-relaxed mb-5">
              {localizedAbout.paragraph1}
            </p>
            <p className="text-gray-500 text-base leading-relaxed mb-8">
              {localizedAbout.paragraph2}
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-10">
              {(localizedAbout.qualities || []).map((q, i) => (
            ...
            <motion.button
              onClick={() => document.querySelector(localizedAbout.buttonLink)?.scrollIntoView({ behavior: 'smooth' })}
